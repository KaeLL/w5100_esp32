 ...
 */
static esp_err_t emac_w5100_write_phy_reg(esp_eth_mac_t *mac, uint32_t phy_addr,
 ...
 */
static esp_err_t emac_w5100_read_phy_reg(esp_eth_mac_t *mac, uint32_t phy_addr,
        uint32_t phy_reg, uint32_t *reg_value)
{
	START;
	END;
...
esp_eth_mac_t *esp_eth_mac_new_w5100(const eth_mac_config_t *mac_config)
{
    ...
    BaseType_t xReturned = xTaskCreate(emac_w5100_task, "w5100_tsk", mac_config->rx_task_stack_size, emac,
